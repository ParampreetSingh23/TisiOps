# One application server: security group, instance, and a stable address.
#
...
#
# Infrastructure only. Installing Docker and Caddy is the bootstrap
# service's job — see services/bootstrap/appServerBootstrap.service.ts.
...
resource "aws_security_group" "app" {
  name        = local.name
  description = "TisiOps managed application server"
  ...
  # range. The application port is deliberately absent: the application is only
  # reachable through Caddy, never directly.
  ingress {
...
resource "aws_instance" "app" {
  ...
  # user_data carries application secrets. IMDSv2 is
...
# both the user's DNS record and any webhook already registered with it.
