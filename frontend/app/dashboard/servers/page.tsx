...
export function StatusDot({ status }: { status: string }) {
  ...
  const isWarning = status === "NEEDS_ATTENTION" || status === "VERIFYING" || status === "PROVISIONING"
  const isErr = status === "UNREACHABLE"
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {visibleServers.map((server) => {
            ...
            const ipDisplay = server.host || server.publicIp || server.elasticIp || "N/A"
      ...
      <ConfirmModal
        ...
        description={`Are you sure you want to remove server "${deleteTarget?.name}" (${deleteTarget?.host || deleteTarget?.publicIp})? This will disconnect the server from your TisiOps workspace.`}
